...
- Overall it is recommended to avoid using Elastic IP, because:
    ...
    - Instead, us a random public IP and register a DNS name to it
...
- Useful for software that have complicated licensing models or for companies that have strong regulatory compliance needs (BYOL feature). 
...
- Images can be customized on runtime with EC2 User data
...
- Advantages of a custom AMI:
    ...
    - Faster boot time (no need for the instance to execute the scripts from the user data)
...
- Stop Hibernat called (Pre-Warm the instance) 
- in the stop hibernat you only pay for the Storage of the EBS . 
- when the instance stopped in hibernate (private ipv4 , ipv6 , running processes , RAM content , instance id) are saved. 
- Public ip also changed with stop hibernat. 

## EC2 Monitoring :

- Basic Monitoring (free) : Metrics send to CloudWatch each 5 minute. 
- Detailed Monitoring (chargable) : Metrics send to CloudWatch each 1 minute.

   **Default Metrics sent to the CloudWatch**
 1- CPU utilization 2- Network utilization 3- Disk performance 4- Disk Read writes 
...
- Permission for SSH key => chmod 0400 (read only for the owner)
